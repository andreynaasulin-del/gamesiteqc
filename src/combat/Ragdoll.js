...
/** Joint, its parent, and the child it points at. The character rig's 25-bone skeleton. */
const JOINTS = [
  ['Hips', null, 'Spine'],
  ['Spine', 'Hips', 'Spine1'],
  ['Spine1', 'Spine', 'Spine2'],
...
/** `rig:LeftArm` and `rigLeftArm` are both `LeftArm`. */
export function stripNamespace(name) {
  return String(name).split(':').pop().replace(/^[a-z]+rig(?=[A-Z_])/i, '');
}
