...
/**
 * The baked noise atlas: half-extent in metres and resolution.
 *
 * The procedural stone used to evaluate ten 3D simplex lookups *per pixel, per
 * frame* — on a floor that covers most of the screen, that was the single most
 * expensive thing in the frame, ahead of every ability. But the floor is flat
 * (y = 0 everywhere), so all of that noise is a function of (x, z) alone and
 * can be rendered exactly once into a texture. 1024² over 128 m is 8 px/m,
 * which resolves the finest term (period 1.4 m) at ~11 px. Past the extent the
 * texture clamps; by then the light pool has sunk the floor to 18 % and the fog
 * has most of the rest, so nothing reads.
 */
const NOISE_EXTENT = 64;
const NOISE_SIZE = 1024;

/** Renders the four noise fields the floor shader used to compute per pixel. */
const noiseBakeShader = {
  uniforms: { uExtent: { value: NOISE_EXTENT } },
  vertexShader: /* glsl */ `
    varying vec2 vUv;
    void main() {
      vUv = uv;
  ...
  fragmentShader: /* glsl */ `
    ...
    void main() {
      ...
      vec3 wp = vec3(xz.x, 0.0, xz.y);
      // Same four terms, same frequencies and offsets as the live shader had.
      float macro = fbm3(wp * 0.018);
      float detail = fbm3(wp * 0.09 + 11.0);
      float fine = snoise01(wp * 0.7);
      float polish = fbm3(wp * 0.06 + 3.0) * 0.5 + 0.5;
      gl_FragColor = vec4(macro * 0.5 + 0.5, detail * 0.5 + 0.5, fine, polish);
    }`
};
...
export class Ground {
  constructor(environment) {
    ...
    /** Baked (macro, detail, fine, polish) noise fields — see `NOISE_EXTENT`. */
    this.noiseTarget = this._bakeNoise(environment.renderer?.gl);
    ...
    this.uniforms = {
      ...
      uTime: { value: 0 },
      uNoiseTex: { value: this.noiseTarget?.texture ?? null },
      uNoiseInvExtent: { value: 1 / (2 * NOISE_EXTENT) }
    };
    ...
    environment.registerShadowCasterWithPatch(this.material, (shader) => {
      ...
      shader.uniforms.uNoiseTex = this.uniforms.uNoiseTex;
      shader.uniforms.uNoiseInvExtent = this.uniforms.uNoiseInvExtent;
      ...
      shader.fragmentShader = shader.fragmentShader
        .replace(
          ...
          `#include <common>
           ...
           uniform float uNoiseInvExtent;
           // Baked noise for this pixel: x macro, y detail (both 0..1, centred
           // on 0.5), z fine 0..1, w polish 0..1. Fetched once in the albedo
           // block and reused by the roughness block below.
           vec4 gGroundNoise;`
        )
        .replace(
          ...
          `#include <map_fragment>
           {
             ...
             gGroundNoise = texture2D(uNoiseTex, clamp(wp.xz * uNoiseInvExtent + 0.5, 0.0, 1.0));
               ...
               float macro = gGroundNoise.x * 2.0 - 1.0;
               float tintMask = smoothstep(-0.5, 0.6, macro);
               vec3 base = mix(uFloorColor, uFloorTint, tintMask * 0.5);
               base *= 1.0 + (gGroundNoise.y * 2.0 - 1.0) * 0.05;
               base *= 1.0 + (gGroundNoise.z - 0.5) * 0.06;
          ...
          `#include <roughnessmap_fragment>
           {
             ...
             float polish = smoothstep(0.3, 0.85, gGroundNoise.w);
  ...
  /**
   * Render the floor's noise fields into a texture, once, at construction.
   *
   * Runs before the floor material compiles so the sampler is bound from the
   * first frame. Returns null without a renderer (tests, SSR); the shader then
   * reads a black texel — flat stone, still correct, just plain.
   */
  _bakeNoise(gl) {
    if (!gl) return null;

    const target = new WebGLRenderTarget(NOISE_SIZE, NOISE_SIZE, {
    ...
    target.texture.name = 'GroundNoise';

    const material = new ShaderMaterial({
      uniforms: noiseBakeShader.uniforms,
      vertexShader: noiseBakeShader.vertexShader,
      fragmentShader: noiseBakeShader.fragmentShader,
      depthTest: false,
      depthWrite: false,
      toneMapped: false
    });
