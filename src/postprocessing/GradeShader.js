...
export const GradeShader = {
  ...
  uniforms: {
    tDiffuse: { value: null },
    // Declared by three's tonemapping chunk; set explicitly so the pass never
    // depends on which material types the renderer uploads it for.
  ...
  fragmentShader: /* glsl */ `
    ...
    void main() {
      ...
      // ---- tone map + display transfer -----------------------------------
      // This pass renders straight to the canvas, so three injects the
      // renderer's tone-mapping operator (toneMapping, with exposure) and the
      // output transfer (linearToOutputTexel). Doing both here replaces the
      // separate OutputPass: one full-resolution read/write fewer per frame.
      // Everything below runs in display space, exactly as it did before.
