...
export class App {
  constructor(canvas) {
    ...
    this.paused = false;
    this._raf = 0;
    /** Embed only: skips frames while the hero card is off screen / inactive. */
    this.presence = new EmbedPresence(canvas);
    ...
    /* ---- adaptive quality ---- */
    /** Frames since start; the contact-shadow throttle keys off its parity. */
    this._frameIndex = 0;
    this._contactEvery = 1;
    this.quality = new QualityGovernor({
      gpu: this.renderer.gpuName,
      onChange: (tier, previous, reason) => this._applyQuality(tier, previous, reason)
    });

    this._bindEvents();
    // Applied after the resize hook is bound (so the post buffers follow the
    // pixel ratio) and before anything is compiled, so the warm-up frames run
    // at the resolution and shadow size the first real frame will.
    this.quality.apply();
  ...
  /**
   * Push one quality tier into every system it touches. See `QualityGovernor`
   * for what each knob costs and why these six were chosen.
   */
  _applyQuality(tier, previous, reason) {
    this.renderer.setPixelRatioCap(tier.pixelRatio);
    this.environment.setShadowMapSize(tier.shadowMap);
    settings.global.particleCount = tier.particleCount;
    settings.global.particleSize = tier.particleSize ?? 1;
    settings.global.volumeSteps = tier.volumeSteps;
    settings.global.surfaceDetail = tier.surfaceDetail ?? 1;
    settings.environment.dustAmount = tier.dustAmount;
    this.abilities.setMaxConcurrent(tier.maxConcurrent);
  ...
  _bindEvents() {
    this._onVisibility = () => { this._wasHidden = true; };
  ...
  _handleAction(action, slot) {
    switch (action) {
      ...
      case 'cancel':
        // Esc puts an armed cast away; a second Esc (nothing armed) hands the
        // page back when we are embedded in the landing card.
        if (this.aim.isArmed) this.aim.cancel();
        else this.presence.requestExit();
        break;
      case 'toggleHelp':
        this.hud.toggleHelp();
        break;
  ...
  armAbility(element = this.element) {
    ...
    // The landing frame is tight on the character; arming eases the rig back
    // so the aim circle and a zone the size of the ward fit on screen. Only
    // ever pulls *out* — a player who has zoomed further stays where they are.
    const cam = settings.camera;
    if (cam.distance < cam.castDistance) cam.distance = cam.castDistance;
  }
  ...
  async load() {
    ...
    // The floor keeps its procedural stone: the photographic scan is 5 MB the
    // short edition does not need (`settings.environment.floorTexture` is off).

    this.loading.setProgress(0.45, 'Loading character…');
    await this.character.load(assets);

    this.loading.setProgress(0.7, 'Loading targets…');
    await this.dummies.load(assets);

    await this._precompile(0.8, 0.99);
  ...
  async _precompile(from, to) {
    ...
    // One more full frame with every ability visible at once, so the shared
    // passes have seen the whole set together before the veil lifts.
  ...
  frame() {
    if (document.hidden || this.presence.suspended) { this._wasHidden = true; return; }
    if (this._wasHidden) {
      this.time.reset();
    ...
    // The governor measures the frame that just finished and may re-tier the
    // pipeline before this one starts, so the change lands cleanly at a frame
    // boundary rather than between passes.
    this.quality.sample(this.time.rawDelta, {
      paused: this.paused,
      // Nothing cast and nothing still burning: cheap frames here prove nothing.
      idle: this.abilities.active.length === 0 && this.decals.active.length === 0
    });
    ...
    // On the low tier the blur passes run every other frame: the shadow under
    // the feet is soft and slow-moving, so a one-frame-stale one is invisible.
    if (this._frameIndex % this._contactEvery === 0) this.contactShadows.render(this.scene);
    ...
    this.hud.update(this.quality.frameMs, this.quality.tier.id);
  }
