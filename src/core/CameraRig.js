...
/** World units the look-at point is lowered by inside the landing card. */
const EMBED_TARGET_DROP = 0.6;
...
export class CameraRig {
  constructor(domElement) {
    ...
    // Low three-quarter view: the character reads tall against the backdrop
    // instead of being looked down on. Distance is overridden by
    // `settings.camera.distance` on the first update; only the angle matters.
    this.camera.position.set(-4.2, 3.0, 8.0);
    ...
    // Also in embed mode: the host only mounts this frame after an explicit
    // Play, so a wheel over the canvas is a zoom, never a page scroll.
  ...
  update(dt) {
    ...
    if (this.camera.aspect < cam.portraitAspect) _desiredTarget.y -= cam.portraitDrop;
    // The landing card is short (≈940×610) and its bottom fifth is the ability
    // bar; looking lower lifts the character clear of it.
    if (EMBED) _desiredTarget.y -= EMBED_TARGET_DROP;
