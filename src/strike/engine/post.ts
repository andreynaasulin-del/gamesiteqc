...
/**
 * The look. Tuned against The editor's viewer render mode: warm afternoon sun, soft contact
...
export function createPostFx(opts: PostFxOptions): PostFx {
  ...
  function build(): void {
    ...
    if (settings.grade.enabled) {
      ...
      // curve still gets to roll the highlights off. (Same shape as The editor's viewer grade.)
      rgb = saturation(
