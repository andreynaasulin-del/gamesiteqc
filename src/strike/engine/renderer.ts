...
/**
 * Fixed physics step. 60 Hz, not 120: a paintball round has no fast projectiles or stacked
 * bodies that need the finer step, and at 120 Hz a machine that draws at 25 fps spends five
 * catch-up substeps — half of its frame — inside the simulation.
 */
export const FIXED_STEP = 1 / 60
/**
 * Three catch-up steps is 50 ms of simulation in one frame. Past that the machine is simply
 * behind and the accumulator is dropped, so a hitch never snowballs into a slideshow.
 */
...
export interface Engine {
  ...
  post: PostFx
  graphics: Graphics
  /** Host visibility gate; skip simulation and every render pass while suspended. */
  isSuspended(): boolean
  /**
   * True only while the player is in a live match. Menus, the team screen and the first frames
   * after loading draw a different, cheaper or stallier scene, and judging the resolution
   * scaler on them costs the player pixels for frames they never played.
   */
  isPlaying(): boolean
  /** Fixed 1/60 s steps, at most 3 per frame. Returns an unsubscribe function. */
...
export async function createRenderer(container: HTMLElement): Promise<Engine> {
  ...
  function frame() {
    timer.update()
    if (document.hidden || engine.isSuspended()) {
      accumulator = 0
      graphics.reset()
      return
    }
    ...
    graphics.sample(timer.getDelta(), !document.hidden && engine.isPlaying())
  }

  function resize() {
    const { w, h } = measure(container)
    renderer.setPixelRatio(graphicsPixelRatio(graphics.quality, window.devicePixelRatio, w, h, graphics.renderScale))
