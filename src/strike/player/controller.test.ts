...
import {
  buildStairCollider,
  buildStairColliderV7,
  STAIR,
  STAIR_V7,
  stairAutopilotYaw,
  stairPoint,
  type StairFixture,
} from '../dev/fixtures/stair-fixture'
...
/**
 * Walks up the real staircase of house.glb (see `dev/fixtures`): a curved flight of ten
...
function climbStairs(input: MoveInput, radius: number, seconds = 6) {
  return climbStair(STAIR, buildStairCollider(), input, radius, { seconds })
}

function climbStairsV7(input: MoveInput, radius: number, opts: { stepHeight?: number } = {}) {
  return climbStair(STAIR_V7, buildStairColliderV7(), input, radius, { seconds: 7, ...opts })
}

/** Walks back down the v7 flight from the Floor-1 slab, on the walk line at `radius`. */
function descendStairsV7(input: MoveInput, radius: number, seconds = 4) {
  const stair = STAIR_V7
  const controller = createCharacterController(buildStairColliderV7())
...
test('walks up the two steps under a 2.48 m ceiling (the map editor storey height)', () => {
...
test('walks up the real the map editor staircase to Floor 1, from any lateral offset', () => {
  for (const radius of [0.6, 0.7, 0.85, 1.0]) {
    const result = climbStairs({ ...idle, forward: 1, walk: true }, radius)
    expect(result.maxY).toBeGreaterThanOrEqual(STAIR.landingY - 0.05)
    expect(result.reachedAt).toBeGreaterThan(0)
  }
})

test('runs and crouches up the real the map editor staircase too', () => {
  for (const radius of [0.6, 0.7, 0.85]) {
    const running = climbStairs({ ...idle, forward: 1 }, radius)
    expect(running.maxY).toBeGreaterThanOrEqual(STAIR.landingY - 0.05)
    const crouching = climbStairs({ ...idle, forward: 1, crouch: true }, radius)
    expect(crouching.maxY).toBeGreaterThanOrEqual(STAIR.landingY - 0.05)
    expect(crouching.airborne).toBe(0)
  }
})

test('the configured step height covers the last riser of the v7 staircase', () => {
  // house v7 has 3 m storeys, so the flight stops 0.50 m under the Floor-1 slab.
  expect(STAIR_V7.finalRiser).toBe(0.5)
  expect(PLAYER.stepHeight).toBeGreaterThanOrEqual(STAIR_V7.finalRiser)
})

test('walks up the v7 staircase and over its 0.50 m last step onto Floor 1', () => {
  for (const radius of [0.6, 0.7, 0.85]) {
    const result = climbStairsV7({ ...idle, forward: 1, walk: true }, radius)
    expect(result.maxY).toBeGreaterThanOrEqual(STAIR_V7.landingY - 0.05)
    expect(result.reachedAt).toBeGreaterThan(0)
...
test('runs and crouches up the v7 staircase too', () => {
  for (const radius of [0.6, 0.7, 0.85]) {
    for (const input of [{ ...idle, forward: 1 }, { ...idle, forward: 1, crouch: true }]) {
      const result = climbStairsV7(input, radius)
      expect(result.maxY).toBeGreaterThanOrEqual(STAIR_V7.landingY - 0.05)
    }
  }
})
...
test('the last riser is taken in one step, not by scrambling over the handrail', () => {
  ...
  const controller = createCharacterController(buildStairColliderV7())
  const [x, z] = stairPoint(STAIR_V7.endAngle - 0.05, 0.85, STAIR_V7)
  ...
  for (let index = 0; index < Math.ceil(1.5 / DT); index++) {
    ...
    controller.update(DT, { ...idle, forward: 1, walk: true },
      stairAutopilotYaw(p.x, p.z, 0.85, 1, STAIR_V7))
    if (controller.state.grounded) heights.add(Math.round(controller.state.position.y * 100) / 100)
  }
  expect(controller.state.position.y).toBeCloseTo(STAIR_V7.landingY, 2)
...
test('walks back down the v7 flight without being pushed up again', () => {
  const result = descendStairsV7({ ...idle, forward: 1, walk: true }, 0.85)
