...
export function startHostAuthority(
  ...
  /**
   * How many of a side's three slots the bot fill may take. A human on the team screen has to
   * have somewhere to go: while anybody is still choosing, the fill leaves one slot free on BOTH
   * sides, so every card on that screen is pickable. Without this the five bots that fit around
   * the chooser packed one side to 3/3, and half the screen answered a click with "full".
   * The held slot is released the moment the last chooser picks, and the fill tops the room up
   * to 3v3 on the very next pass (`requestTeam` rebalances on the spot).
   */
  const botSeatsPerTeam = (): number => {
    for (const hp of players.values()) if (!hp.isBot && !hp.team) return MATCH.teamSize - 1
  ...
  const balance = () => {
    ...
    // Read once per pass, after the humans above have been counted: a chooser holds a slot on
    // each side, and a side already carrying three humans caps itself below.
    const botSeats = botSeatsPerTeam()
    for (const hp of bots) {
      let team = hp.team
      if (!team) {
        team = pickTeam(fakeCount(counts))
        if (counts[team] >= botSeats) team = otherTeam(team)
      }
      if (counts[team] >= botSeats) {
        // Its side is full — a human took the slot, by joining or by swapping. The bot is
        // KICKED, never moved across: the netcode delivers a bot's state to the other
    ...
    // places, so filling up to `counts.a + counts.b` would ask the netcode for a seventh
    ...
    if (
      fill &&
      players.size < MATCH.maxPlayers &&
      // A seat in the room is not a slot on a team: with a chooser holding one slot a side, the
      // sixth seat stays empty until they pick, or the bot we added would be kicked by the pass
      // above on the next tick, forever.
      (counts.a < botSeats || counts.b < botSeats) &&
      !addingBot
    ) {
      addingBot = true
      room
  ...
  // except that the netcode only sends that state when the bot joins (see `GS.botStats`). So the
  ...
  const tick = () => {
    ...
    // client writing its state, and the state the netcode replayed for it when it joined can be
