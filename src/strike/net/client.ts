...
/**
 * The read side of the network: turns the netcode players + RPCs into registry entities and
...
/** How often we re-read the reliable per-player states (the netcode has no per-key change event). */
...
export function bindNetToRegistry(
   ...
   *
   * the netcode hands us a bot's player state exactly once, when it joins, and that snapshot can
  ...
  const poll = () => {
    ...
    applyBotStats()
    // Anyone the registry still knows but the netcode dropped (missed onQuit). Not gated on the
