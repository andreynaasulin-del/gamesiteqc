...
export const GS = {
   ...
   *
   * Not a boolean on the wire on purpose: the netcode never delivers a global whose
   ...
   *
   * A bot is a real participant with its own state, but the netcode only ever delivers
...
export const RPCS = {
  ...
  team: 'team',
  /**
   * ALL — TeamResult. the netcode has no "reply to one player" mode and our `rpc.register`
   * wrapper drops handler return values, so the host's answer is a broadcast that every
   * client filters by `player`.
   */
  teamResult: 'teamResult',
  /**
   * ALL — HitRejected, filtered by `player` the way `teamResult` is (the netcode cannot answer one
...
/** How a call is routed. Mapped onto the netcode's numeric `RPC.Mode` inside `room.ts`. */
 ...
 *
 * A netcode socket hiccup flipped a guest's `isHost()` to true for about a second; it started a
...
export interface DoorEvent {
  /** `DoorInfo.id` (id of the door/window node). */
  id: string
  open: boolean
...
/**
 * The host threw one of your hits away, and why. Reaches every client (the netcode has no reply-to-
