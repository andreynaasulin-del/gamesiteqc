...
/**
 * Called the moment a new avatar enters the scene, so its GPU programs can be built off the
 * critical path. Optional: the tests and headless callers run without a renderer.
 */
export type AvatarAddedHook = (object: Object3D) => void

export function createRemotePlayers(scene: Scene, registry: EntityRegistry, onAvatarAdded?: AvatarAddedHook): RemotePlayers {
