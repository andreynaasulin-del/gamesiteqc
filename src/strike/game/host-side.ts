 ...
 * no authority exists, so nothing is registered for the `hit` RPC — while the netcode, whose own
...
export function createHostSide(opts: HostSideOptions): HostSide {
   ...
   * registers its own handler, and the netcode's RPC bus warns past ten listeners on one room —
  ...
  async function startBots(): Promise<void> {
    ...
    bots = create({
      ...
      onSnapshot: (bot, snapshot) => {
        // The runner reuses one snapshot object per bot, so copy before handing it to the netcode.
  ...
  function start(): void {
    ...
    // Off the bus *before* the authority registers its own `hit` handler, not after: the netcode
