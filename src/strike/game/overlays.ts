 ...
 *
 * Everything here is plain DOM on the shared design tokens from `ui/styles.css`; the in-match
...
export function createTeamScreen(opts: TeamScreenOptions): TeamScreen {
  ...
  const card = (choice: TeamChoice, index: number): HTMLButtonElement => {
    ...
    // Three heads a side is the whole room, so a side at 3/3 is the normal state of this
    // screen - and it read as "no space, go away". This line says what actually happens.
    const hint = el('span', { class: 'ps-pick-hint' })
    ...
    const node = el(
      ...
      [
        ...
        choice === 'auto' ? autoHint : el('span', { class: 'ps-pick-state' }, [count, hint]),
        list,
      ],
    )
  ...
  const tagline = el('p', {
    class: 'ps-tagline',
    html: 'Pick a side and drop in. <b>1</b> / <b>2</b> / <b>3</b> or <b>ENTER</b>.',
  })
  ...
  const renderRoster = () => {
    ...
    for (const team of TEAM_KEYS) {
      ...
      const free = MATCH.teamSize - members.length
      const bots = members.filter((m) => m.isBot).length
      const mine = opts.myTeam() === team
      // A bot is filler: the host kicks one to seat a human, so a side full of bots is still
      // open. A side full of PEOPLE is the only one that can turn a click down.
      hints[team].textContent = mine
        ? 'Your team'
        : free > 0
          ...
          : bots > 0
            ? 'A bot steps aside'
            : 'Full - humans only'
      cards[team].classList.toggle('is-full', free <= 0 && bots === 0 && !mine)
 ...
 * includes the 30 m terrain plane baked into the map, which would push the circle out to 25 m and 15 m up —
...
export function createPauseMenu(opts: PauseMenuOptions): PauseMenu {
  ...
  const leave = el('button', { class: 'ps-btn ps-btn--ghost ps-btn--block' }, [
    ...
    // in the hero card it returns the page.
    EMBED ? 'Back to the page' : 'Restart match',
  ])
  ...
  const renderMaps = () => {
    ...
    for (const builtin of BUILTIN_MAPS) {
      const card = el('button', { class: 'ps-map', type: 'button' }, [
        ...
        el('span', { class: 'ps-map-meta', text: 'Built-in map' }),
      ])
   ...
   * bot the other way when a human swaps, so the score of the room stays true through the swap.
   *
   * It reads "Orange · 3 v 3", never "3/3": a capacity out of a maximum on the row that also
   * says which side you are on is read as "the room is full, you are not getting in" — the one
   * thing this line must never say to someone who is already playing.
   */
  const renderTeams = () => {
    ...
    teamLabel.textContent = mine
      ? `${TEAMS[mine].name} · ${counts[mine] ?? 0} v ${counts[mine === 'a' ? 'b' : 'a'] ?? 0}`
      : 'Choosing…'
  ...
  resume.addEventListener('click', () => {
    played = true
    // Only pointerlockchange confirms launch and closes the menu.
    opts.onResume()
  })
