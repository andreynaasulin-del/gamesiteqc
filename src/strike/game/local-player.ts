...
export function createLocalPlayer(opts: LocalPlayerOptions): LocalPlayer {
  ...
  /**
   * Are the controls ours to read? Either scheme counts: a captured pointer, or the drag
   * fallback used where the browser refuses the capture (an embedded frame, Safari). Gating
   * this on `input.locked` alone is what used to leave the player standing still in the card
   * on the landing page — the match ran, the bots played, and nothing answered the keyboard.
   */
  const controlsLive = () => input.locked || input.engaged
  ...
  const player: LocalPlayer = {
    ...
    fixedUpdate(dt) {
      ...
      const source = overrideMove ? override : controlsLive() ? input.move : ZERO_MOVE
    ...
    frameUpdate(dt) {
      ...
      const dx = (controlsLive() ? mouse.dx : 0) + pendingLookX
      const dy = (controlsLive() ? mouse.dy : 0) + pendingLookY
      pendingLookX = 0
      pendingLookY = 0
      if (spectating) {
