/**
 * The real staircase of `house.glb`, frozen into fixtures so headless tests can walk the
 * geometry the game actually builds (see `extract-stair.ts` for how it was pulled out).
 *
 * The editor's "Staircase 1" is a CURVED flight: ten 0.25 m risers sweeping 180° of an arc centred
...
import fixtureV5 from './stair-fixture.json'
import fixtureV7 from './stair-fixture-v7.json'
...
/** house v5: the flight ends level with Floor 1. */
export const STAIR: StairFixture = { ...ARC, landingY: 2.55, finalRiser: 0 }

/** house v7: 3 m storeys, so the top tread is 0.50 m below the Floor-1 slab. */
export const STAIR_V7: StairFixture = { ...ARC, landingY: 3.05, finalRiser: 0.5 }

/** Builds the collider once per call (tests mutate nothing, but each gets its own BVH). */
export function buildStairCollider(): StaticCollider {
  return buildCollider(fixtureV5.positions, 'stair-fixture-v5-fixture')
}

export function buildStairColliderV7(): StaticCollider {
  return buildCollider(fixtureV7.positions, 'stair-fixture-v7-fixture')
}
