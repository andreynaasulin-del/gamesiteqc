/**
 * One-off extractor (W4-E): pull real collider triangles out of a loaded the map editor map so a
 ...
 *
 * The result is written to `stair-fixture.json` next to this file.
 */
...
export interface ExtractOptions {
  /** Defaults to the built-in the map editor house. */
  mapUrl?: string
...
export async function extractColliderTriangles(opts: ExtractOptions): Promise<ColliderFixture> {
  const mapUrl = opts.mapUrl ?? '/maps/house.glb'
