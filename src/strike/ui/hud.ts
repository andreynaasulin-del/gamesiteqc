...
export interface Hud {
  ...
  /**
   * The line under the top bar: the pointer is free, or the capture was refused and the game
   * runs on drag-look. `text` replaces the default wording (the fallback explains itself).
   */
...
const PAINT_MS = 1200
/** Default wording of the pointer hint: the pointer is free and the match is waiting. */
const POINTER_FREE_HINT = 'Pointer free · click the game or press P to resume'
...
export function createHud(mount: HTMLElement = appRoot()): Hud {
  ...
  const pointerHint = el('div', { class: 'ps-pointer-hint', text: POINTER_FREE_HINT, hidden: true })
  ...
  const hud: Hud = {
    el: root,
    setPointerReleased(free, text) {
      pointerHint.textContent = text ?? POINTER_FREE_HINT
      pointerHint.hidden = !free
    },
    setArmor(value) {
    ...
    setRoom(code, url) {
      ...
      inviteUrl = url
      // Offline there is no room to join and no link to copy. A chip reading "ROOM ----"
      // is a dead control in the corner of every frame: hide it rather than explain it.
      roomChip.hidden = !code
    },
    setSpread(spread) {
