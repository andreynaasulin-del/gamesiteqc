...
import { DEFAULT_CHARACTERS } from './catalog'
import { mergeCharacterSkins } from './merge-skins'

async function fetchCharacterResource(url: string): Promise<Response> {
  const response = await fetch(url, { signal: AbortSignal.timeout(30000) })
  if (!response.ok) throw new Error(`Could not load character (${response.status}). Try again.`)
  return response
}
...
export function loadCharacterAsset(character: CharacterSelection): Promise<CharacterAsset> {
  ...
  if (!pending) {
    pending = (async () => {
      ...
      if (!preset) throw new Error('Unknown character.')
      const response = await fetchCharacterResource(character.manifestUrl)
      const manifest = await response.json()
      if (manifest.schema !== 'quadcode.character.v1' || manifest.gender !== character.gender) throw new Error('Unsupported character manifest.')
      const modelUrl = assetUrl(`characters/${preset.id}.glb`)
      // The rig contract shares one animation library per body type.
      const animationUrl = assetUrl(`characters/${character.gender}-animations.glb`)
...
export function instantiateCharacter(asset: CharacterAsset): CharacterInstance {
  ...
  // A wardrobe of 23 separate skinned parts is a wardrobe of 23 draw calls. Weld the parts that
  // render alike (their flat colours move onto the vertices) and put frustum culling back on.
  const welded = mergeCharacterSkins(model)
  const dropped = new Set(welded.retired)
  for (let i = materials.length - 1; i >= 0; i--) if (dropped.has(materials[i])) materials.splice(i, 1)
  welded.retired.forEach(material => material.dispose())
  welded.merged.forEach(mesh => materials.push(mesh.material as MeshStandardMaterial))
