 ...
 *
 * Map-editor exports one node per building element: house is 125 glTF meshes but ~1,700
 ...
 * - meshes with children — hiding those would hide the children too, and the map editor only puts
 ...
 * glass still reads correctly. Identity nodes (anything with the map editor `extras`) stay in the tree
...
import type { DoorInfo, SceneExtras } from '../types'
...
export function batchStaticMeshes(root: Object3D, excluded: Set<Object3D>[]): BatchStats {
  ...
  for (const group of groups.values()) {
    ...
    for (const mesh of merged) {
      // Identity nodes carry the map editor registry; other modules look objects up through them.
      if (typeof (mesh.userData as Partial<SceneExtras> | undefined)?.kind === 'string') {
        mesh.visible = false
      } else {
...
function wantedAttributes(material: Material): string[] {
  ...
  // glTF TEXCOORD_n maps to three's `uv`, `uv1`, `uv2`, `uv3`. Map-editor bakes ambient occlusion
 ...
 *
 * Everything goes through `fromBufferAttribute`/`getX`, which dequantises: Map-editor GLBs use
...
function bakeVisualGeometry(mesh: Mesh, wanted: string[], matrix: Matrix4): BufferGeometry | null {
  ...
  // A mesh missing a secondary UV set the material samples (a few the map editor primitives have no
 ...
 * house is 128 leaf meshes across 9 openables and only 30 (animated node × material)
