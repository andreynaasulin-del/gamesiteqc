 ...
 *
 * The editor's `kind: 'spawn'` node is the project's single walkthrough start marker, not a team
...
function spawnsFromZones(map: MapData, world: WorldQuery, center: Vector3): SpawnLayout | null {
  ...
  // Interleave areas: a team may have several Spawn A/B zones (e.g. both lanes in the ice yard).
...
function autoSpawns(map: MapData, world: WorldQuery, nav?: Navigation): SpawnLayout {
  ...
  // around a map-editor house is only a fallback for maps that have no enclosed space at all.
  ...
  if (startMarker) {
    // The editor's walkthrough start marker is not a team spawn, but it is a good hint for where
