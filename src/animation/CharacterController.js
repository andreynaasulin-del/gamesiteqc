...
/** The character rig exports in centimetres. */
 ...
 * clips ship as separate rig exports of the *same* skeleton, so only their
...
export class CharacterController {
  ...
  _measureFacing(root) {
    ...
    root.traverse((node) => {
      ...
      // Exporters disagree on the namespace: "rig:LeftFoot", "rigLeftFoot".
      const short = node.name.split(':').pop().replace(/^[a-z]+rig(?=[A-Z_])/i, '');
