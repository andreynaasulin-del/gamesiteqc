...
const PARTICLE_FRAGMENT = /* glsl */ `
  ...
  float shapeMask(vec2 uv, out float hilite) {
    ...
    #elif SHAPE == 1                     // SMOKE
      // Two octaves, not three. A smoke puff is a soft blob with a ragged
      // edge: the first octave is the blob, the second is the rag, and the
      // third was a 2 px ripple that the soft-fade and bloom blurred out
      // anyway. This shape is the single most expensive fragment in the game
      // by area (the quake's dust covered a third of the screen with it), and
      // the octave was a third of its cost.
      vec3 np = vec3(c * 1.6, vSeed * 21.0 + uTime * 0.25);
