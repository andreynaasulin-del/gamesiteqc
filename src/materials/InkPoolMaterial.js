...
const POOL_FRAGMENT = /* glsl */ `
  ...
  uniform float uDetail;   // 1 = every term; 0 = the low tiers: no sub-pixel grain
  ...
  void main() {
    ...
    // The quad is 16 m across; the wash rarely reaches 10. Before this, every
    // pixel of the empty border paid four noise calls to learn it was empty.
    // The bound is the deckle at its fullest (deckle = 1), so nothing a later
    // line could draw is ever thrown away here.
    float aa = fwidth(rad) + 0.01;
    float washMax = uRadius * uWashRadius * (1.0 + uWashDeckle * 0.5);
    if (rad > max(washMax + uWashBleed, washMax * uSplatterSpread) + aa * 6.0) discard;
    ...
    // On the low tiers the fine terms are off outright: the pixel is already
    // 1/0.85 of a device pixel wide there, so this is the same fade, taken
    // early. Every term scaled by detail below is also skipped when it is 0,
    // which is nine noise calls per pixel of a sixteen-metre quad.
    float detail = uDetail > 0.5 ? 1.0 - smoothstep(0.02, 0.13, footprint) : 0.0;
    bool fine = detail > 0.001;
    ...
    /* ---- the paper ---- */
    float tooth = fine ? fbm3(vec3(p * uWashToothScale, uSeed * 3.0)) * 0.5 + 0.5 : 0.5;
    ...
    float grain = fine ? fbm3(vec3(p * uGranulationScale, uSeed * 11.0)) * 0.5 + 0.5 : 0.5;
    ...
    // Wet-only terms are skipped on the dry paper, which is most of the quad.
    bool inWater = water > 0.001;
    float strand = inWater ? 1.0 - abs(snoise(vec3(ip * uInkVeilScale, uSeed * 5.0 - uTime * 0.06))) : 0.0;
    ...
    float chop = fine ? snoise(vec3(p * uChopScale, uTime * uRippleSpeed * 1.7 + uSeed * 3.0)) : 0.0;
    ...
    if (fine && inWater) {
      ...
      caustic = pow(clamp(1.0 - abs(c1 + c2) * 0.9, 0.0, 1.0), 6.0) * uCaustic * water * detail;
    }
    ...
    float foamBreak = foamBand > 0.001 ? snoise01(vec3(dir * 7.3, uTime * 0.5 + uSeed)) * 0.7 + 0.3 : 0.0;
...
export function createInkPoolMaterial() {
  ...
  material.userData.sync = (state) => {
    ...
    u.uDetail.value = g.surfaceDetail ?? 1;
