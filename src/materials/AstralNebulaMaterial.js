...
const NEBULA_FRAGMENT = /* glsl */ `
  ...
  void main() {
    ...
    // Intersect the actual oblate envelope before evaluating any noise.
    // Keep the sphere bound too: it matches the existing proxy geometry.
    float outer = max(0.001, mix(1.0, uCavity * 1.3, uDrain));
    vec3 axes = max(uRadius, 1e-3) * outer * vec3(1.0, max(uFlatten, 0.05), 1.0);
    vec3 eo = (ro - uCentre) / axes;
    vec3 ed = rd / axes;
    float aa = dot(ed, ed);
    float bb = dot(eo, ed);
    float cc = dot(eo, eo) - 1.0;
    float disc = bb * bb - aa * cc;
    if (disc <= 0.0) discard;
    float root = sqrt(disc);
    float t0 = max(max(ta, 0.0), (-bb - root) / aa);
    float t1 = min(tb, (-bb + root) / aa);
...
export function createAstralNebulaMaterial() {
  ...
  material.userData.sync = (state) => {
    ...
    // Floor 6, matching the shader's own clamp. A floor of 10 made the eco
    // tier's `volumeSteps` a dead knob: 24 × 0.4 = 9.6 was pushed back up to
    // 10 samples of six noise calls each, ~24 ms of GPU on an M1 for one nebula.
    u.uSteps.value = Math.max(6, Math.round(c.nebulaSteps * (g.volumeSteps ?? 1)));
