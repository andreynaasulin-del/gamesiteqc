...
const STONE_PARS = /* glsl */ `
  ...
  uniform float uDetail;   // 1 = full weathering, 0 = one octave of it

  /**
   * The weathering noise, at the octave count the quality tier can afford.
   * A uniform branch is coherent across the whole draw, so this costs one
   * compare — and on the cheap path two of every three snoise calls.
   */
  float weather(vec3 p) {
    if (uDetail > 0.5) return fbm3(p);
    return snoise(p) * 0.5;
  }
  ...
  Stone sampleStone(vec3 wp, vec3 wn, float seed) {
    ...
    // Only worth sampling while the maps are still loading: once uTexAmount
    // reaches 1 the mix below discards it entirely.
    float macro = uTexAmount > 0.98 ? 0.5 : weather(wp * 0.6 + seed * 13.0) * 0.5 + 0.5;
  ...
  float dustCoverage(vec3 wp, vec3 wn) {
    ...
    float mottle = weather(wp * uDustScale) * 0.5 + 0.5;
...
function syncStone(uniforms, c, g) {
  ...
  uniforms.uDetail.value = g.surfaceDetail ?? 1;
}
          ...
          `#include <map_fragment>
           {
             ...
             float streak = weather(vec3(vRiftWorld.xz * 2.6, vRiftWorld.y * 0.6)) * 0.5 + 0.5;
