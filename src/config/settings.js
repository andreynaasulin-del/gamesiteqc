...
export const settings = {
  ...
  /* ------------------------------------------------------------------ */
  quality: {
    // Hard ceiling on rendered pixels per frame; see Renderer.PIXEL_BUDGET
    // for the measurements behind the number. Tiers scale everything else.
  ...
  global: {
    ...
    // 0 or 1. Set by the quality governor. At 0 the surface shaders that
    // layer procedural weathering on top of textures (the rift's monoliths)
    // drop their multi-octave noise to a single octave. The maps still do the
    // work; what goes is the second and third octave of grime nobody sees on
    // an integrated GPU rendering at 0.85 pixel ratio.
    surfaceDetail: 1
  },
  ...
  acid: {
    ...
    mistSteps: 18, // samples per pixel through the volume; the march is jittered, so 18 reads as smooth
  ...
  ink: {
    ...
    wispSteps: 16, // samples per pixel through the volume (jittered march)
    wispDensity: 3.4,
  ...
  astral: {
    ...
    nebulaSteps: 24, // samples per pixel through the cloud (jittered march; the biggest single GPU dial in the game)
    nebulaDensity: 2.4,
  ...
  /* ------------------------------------------------------------------ */
  camera: {
    // The landing frame: close and low, so the character fills the screen
    // instead of standing in the middle of an empty floor. The first cast pulls
    // the rig back to `castDistance` so a zone cast fits (see App#_cast).
    distance: 6.0,
    castDistance: 10.5,
    minDistance: 3.5,
    maxDistance: 30,
    ...
    fov: 42,
    targetHeight: 1.1,
    // Portrait screens: the ability bar and the onboarding sheet own the lower
    // half, so the look-at point drops and the character rides up into the
    // clear part of the frame. Metres, subtracted from `targetHeight`.
    portraitDrop: 0.9,
    portraitAspect: 0.8,
    damping: 0.06,
  ...
  environment: {
    ...
    rimAzimuth: 5.45,
    rimElevation: 0.35,
    // Hero light: a warm coral spot parked above and in front of the character.
    // It is the brand colour, it lifts the face and the jacket off the floor,
    // and its cone leaves a pool of light on the stone under the caster so the
    // eye lands there first. Follows the character; no shadows.
    heroIntensity: 80,
    heroColor: '#ff9569',
    heroHeight: 3.8,
    heroOffset: 2.2, // metres toward the camera side
    heroAngle: 0.7, // cone half-angle, radians
    heroPenumbra: 0.8,
    envIntensity: 0.32,
 ...
 * light. Slot order is MOBA order: Q W E R on the left hand, D F for the last
 * two, and 1–6 mirror them.
 */
export const ELEMENTS = ['ward', 'venom', 'astral', 'quake', 'ink', 'rend'];
...
export const ELEMENT_META = {
  ward: {
    ...
    accent: '#ff9569',
    key: 'Q',
    hint: 'Volcanic Ward — drop a burning zone',
    blurb: 'Far cast. Drop a ring of fire and obsidian anywhere on the floor.',
    cast: CastShape.ZONE
  },
  venom: {
    label: 'Venom Surge',
    accent: '#a878f0',
    key: 'W',
    hint: 'Venom Surge — fire a line of crystal',
    blurb: 'Line cast. Swing the arrow, fire a seam of amethyst down it.'
  },
  astral: {
    label: 'Void Blast',
    accent: '#dd344d',
    key: 'E',
    hint: 'Void Blast — open a singularity',
    blurb: 'Far cast. Open a black hole. Whatever it catches, it keeps.',
    cast: CastShape.ZONE
  },
  quake: {
    label: 'Monolith Rift',
    accent: '#d9c6a3',
    key: 'R',
    hint: 'Monolith Rift — split the floor',
    blurb: 'Line cast. Crack the floor open and heave slabs of stone out of it.'
  },
  ink: {
    label: 'Sumi Tide',
    accent: '#5fd3c8',
    key: 'D',
    hint: 'Sumi Tide — paint a whirlpool',
    blurb: 'Far cast. Paint a whirlpool on the floor and pull the targets into it.',
    cast: CastShape.ZONE
  },
  rend: {
    ...
    accent: '#ffcf7a',
    key: 'F',
    hint: 'Celestial Rend — call down judgment',
    blurb: 'Far cast. Call down a column of light. It takes what stands in it.',
    cast: CastShape.ZONE
  }
};
