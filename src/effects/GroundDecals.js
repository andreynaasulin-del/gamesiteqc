...
const DECAL_FRAGMENT = /* glsl */ `
  ...
  void main() {
    ...
    float fadeOut = 1.0 - smoothstep(0.55, 1.0, uAge);
      ...
      // Nothing past the ring's edge is ever visible; leave before the noise.
      if (d > radius) discard;
      // Two octaves. Settled dust on a floor is a soft smear; the third octave
      // was sub-pixel at the 0.85 ratio the low tiers render at, and the
      // DUSTRING is by far the most-drawn decal (the rift lays dozens).
      vec3 np = vec3(c * 3.1, uSeed * 7.0 + uTime * 0.2);
      float n = snoise(np) * 0.5 + snoise(np * 2.02) * 0.25;
...
export class DecalSystem {
  constructor(scene) {
    ...
    // How many marks may lie on the floor at once. Set per quality tier. One
    // rift left forty rings up to sixteen metres wide, every pixel of every
    // one running three octaves of noise — half the GPU cost of the cast.
    this.maxActive = Infinity;
  }

  /** Cap the live count; the oldest marks retire first when it is exceeded. */
  setBudget(max) {
    this.maxActive = Math.max(1, max ?? Infinity);
    this._enforceBudget(0);
  }

  _enforceBudget(incoming) {
    while (this.active.length + incoming > this.maxActive && this.active.length > 0) {
